...
var logger = {
  ...
  log: function(msg) {
    if (IsNode)
      process.stdout.write(
        "[ " + chalk.blue("INFO") + " ] " + chalk.blue(msg) + "\n"
      );
  ...
  warn: function(msg) {
    if (IsNode)
      process.stdout.write(
        "[ " + chalk.yellow("WARN") + " ] " + chalk.yellow(msg) + "\n"
      );
  ...
  ok: function(msg) {
    if (IsNode)
      process.stdout.write(
        "[ " + chalk.green("SUCCESS") + " ] " + chalk.green(msg) + "\n"
      );
...
const cdn =
  window.localStorage.getItem("url-cdn") || "https://cdn.discordapp.com";
const endpoint =
  window.localStorage.getItem("url-api") || "https://discordapp.com";
const inviteBase =
  window.localStorage.getItem("url-invite") || "https://discord.gg";
...
function loadPlugins(dir, callback) {
  if (!IsNode) {
    if (callback)
      return callback("Plugins are only supported on the desktop app");
  ...
  fs.readdir(dir, function(err, files) {
    ...
    files.forEach(function(plugin, i) {
      if (plugpaths.includes(plugin))
        return logger.debug("Skipping already-loaded plugin: " + plugin);
      ...
      for (var hk in pl.hooks) {
        ...
        switch (hook.trigger) {
          ...
          default: {
            // We'll add more hooks soon™ but for now, this seems useful
...
function parseDiscordEmotes(content) {
  ...
  for (var itm in arr) {
    ...
    var link = `${cdn}/emojis/${colsplit[colsplit.length - 1].substring(
      0,
      colsplit[colsplit.length - 1].length - 4
    )}.png`;
    var imgnode = document.createElement("img");
...
function createLinksAndImages(content, images) {
  var temphtml = content.innerHTML
    .replace(/```.*```/g, "")
    .replace(/`.*`/g, ""); // Big hack...
...
function parseMarkdown(content, maskedLinks) {
  ...
  for (var i in underlined) {
    ...
    txt = txt.replace(
      match,
      "<underline>" + match.replace(/\_\_/g, "") + "</underline>"
    );
  }
  ...
  for (var i in italics) {
    ...
    txt = txt.replace(
      match,
      "<i>" + match.replace(/\*/g, "").replace(/\_/g, "") + "</i>"
    );
  }
  ...
  for (var i in codeblock) {
    ...
    finaltext = `<code class="codeblock ${lang}">${finaltext.replace(
      /^<br>+|<br>+$/gm,
      ""
    )}</code>`;
  ...
  if (maskedLinks) {
    ...
    var masked = txt.match(
      /\[.*\]\((http|https):\/\/([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:/~+#-]*[\w@?^=%&/~+#-])?\)/gi
    );
    for (var i in masked) {
...
function addMessageToDOM(msg, complete) {
  ...
  title.textContent =
    msg.guild.members.get(msg.author.id).displayName +
    (msg.author && msg.author.bot ? " [BOT]" : "");
  ...
  $(content)
    ...
    .click(function() {
      ...
      if (
        link.match(new RegExp(inviteBase + "/" + "[A-Za-z0-9]*", "gi"))[0] ==
        link
      ) {
...
function loadTypingUsers() {
  ...
  if (len == 0) text = "";
    ...
    text = typingArr[0] + ", " + typingArr[1] + ", and " + typingArr[2];
...
function BotListeners() {
  ...
  bot.on("voiceStateUpdate", function(oldmember, newmember) {
    if (newmember.guild.id != bot.channels.get(window.channelID).guild.id)
      return;
      ...
      container
        ...
        .replaceChild(
          ...
          document.body.querySelectorAll(
            `div#msg-${oldmsg.id} > div > div.content`
          )[0]
        );
      container
        .getElementsByTagName("div")[0]
        .replaceChild(
          images,
          document.body.querySelectorAll(
            `div#msg-${oldmsg.id} > div > div.images`
          )[0]
        );
    });
  });
  ...
  bot.on("ready", function() {
    ...
    window.channelID =
      window.localStorage.getItem("lastchannel") ||
      bot.channels
        .filter(function(a) {
          return a.type == "text";
        })
        .first().id;
    if (!bot.channels.get(window.channelID))
      window.channelID = bot.channels
        .filter(function(a) {
          return a.type == "text";
        })
        .first().id;
  ...
  bot.on("disconnect", function(err) {
    ...
    $("#loading-line").html(
      window.loading_lines[
        Math.floor(Math.random() * window.loading_lines.length)
      ]
    );
...
function voice(voiceChannelID) {
  if (!IsNode)
    return logger.warn("This is only supported on the desktop version!");
  ...
  chan
    ...
    .then(function(connection) {
      ...
      connection.on("speaking", function(user, speaking) {
        var usernode = document.getElementById(user.id);
        if (!speaking)
          return (usernode.classList = usernode.classList.value.replace(
            /( |)voice-speaking/g,
            ""
          ));
        receiver.createPCMStream(user).pipe(new Speaker(), { end: false });
...
$(document).ready(function() {
  ...
  $("#create-channel").click(function() {
    ...
    bot.channels
      .get(window.channelID)
      .guild.createChannel(newChannelName.value)
  ...
  $("#create-server").click(function() {
    bot.user
      .createGuild($("#new-server-name").val())
      .then(function(guild) {
        $.modal.close();
  ...
  $(".twemoji-textarea").on("keydown", function(e) {
    ...
    if (keyCode == "13" && !e.shiftKey) {
      // We ignore enter key if shift is held down, just like the real client
      e.preventDefault();
      ...
      chan.send(
        findEmojis(
          $("<pre>")
            .html(messageInput.innerText.replace(/<br>/gi, "\n"))
            .text()
        )
      );
        ...
        case "invite": {
          bot.channels
            ...
            .then(function(invite) {
              $("#display-invite-modal > span#invite-text").text(
                `${inviteBase}/${invite.code}`
              );
              $("#display-invite-modal > h2 > span#server-name").text(
                invite.guild.name
              );
    ...
    callback: function(key, options) {
      switch (key) {
        case "ban": {
          bot.channels
            .get(window.channelID)
            .guild.members.get(options.$trigger[0].id)
            .ban()
            .then(function() {
  ...
  var contextOptions = {
    ...
    callback: function(key, options) {
      ...
      var messageContent = document.querySelector(
        `#${options.$trigger[0].id} > .message-inner > .content`
      );
          ...
          $(messageContent).on("keydown", function(e) {
            ...
            if (keyCode == "13" && !e.shiftKey) {
              ...
              bot.channels
                .get(window.channelID)
                .messages.get(messageId)
                .edit(messageContent.textContent)
                .then(function() {
...
function ChannelChange(channelID, silent, force) {
  ...
  if (!silent) {
    ...
    changemsg.innerHTML = `Changed to #${channel.name} on ${server.name} - ${
      channel.topic ? channel.topic : "<no channel topic>"
    }`;
...
function loadMembers(memb) {
  ...
  if (
    memb &&
    memb.id &&
    !bot.channels.get(window.channelID).guild.members.get(memb.id)
  )
    return;
  ...
  mem.forEach(function(user, i) {
    ...
    if (user.user.presence.game) {
      ...
      playing.textContent = `${
        user.user.presence.type == 1 ? "Streaming" : "Playing"
      } ${user.user.presence.game.name}`;
...
function loadMessages(hideLoaderAfter) {
  ...
  if (
    window.currentMessages.channelID == window.channelID &&
    window.currentMessages.arr.length > 0
  )
    options.before = window.currentMessages.arr[0].id;
  bot.channels
    .get(window.channelID)
    ...
    .then(function(messages) {
      ...
      if (len <= 0) {
        ...
        if (scrolltobottom)
          msgdom.scrollTop = msgdom.scrollHeight - oldScrollHeight;
      ...
      messages.forEach(function(curmsg, i) {
        ...
        addMessageToDOM(curmsg, function(items) {
          ...
          container.classList = "message";
          if (curmsg.author.id == bot.user.id)
            container.classList += " my-message";
          msgobj.classList = "message-inner";
          container.appendChild(msgobj);
...
function loadServers() {
  ...
  var srvlist =
    bot.user.settings &&
    bot.user.settings.guildPositions &&
    bot.user.settings.guildPositions.length > 0
      ? bot.user.settings.guildPositions
      : Array.from(bot.guilds.keys());
  srvlist.forEach(function(srv, i) {
    ...
    if (!servericon)
      servericon =
        !server.available || !server.name || !server.name.match(/\b(\w)/g)
          ? "unavailable.png"
          : "https://dummyimage.com/256x256/ffffff/000000.png&text=" +
            encodeURI(server.name.match(/\b(\w)/g).join(""));
...
function loadChannels(chan) {
  if (chan && chan.guild.id != bot.channels.get(window.channelID).guild.id)
    return;
...
function loadVoiceMembers(channelID, container, cb) {
  ...
  chan.members.forEach(function(member, indx) {
    ...
    memnode.classList =
      "voice-user" +
      (member.deaf ? " voice-deaf" : "") +
      (member.mute ? " voice-mute" : "") +
      (member.speaking ? " voice-speaking" : "");
    var avatar = document.createElement("div");
